...
Exposes PySpark ingestion, PyTorch Vector Search, and AWS Bedrock model routing
...
def rag_search_knowledge_base(query: str, category_filter: str = None) -> str:
    """
    Executes RAG retrieval over PyTorch Vector Store and synthesizes an answer using AWS Bedrock routing.
    """
...
@server.tool()
def bedrock_route_prompt(prompt: str) -> str:
    """
    Evaluates prompt complexity and routes directly to the optimal AWS Bedrock LLM model.
    """
    result = route_prompt_to_bedrock(prompt=prompt)
    return json.dumps(result, indent=2)
